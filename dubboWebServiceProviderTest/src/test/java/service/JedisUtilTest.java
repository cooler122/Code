...
public class JedisUtilTest {
	...
	@Before
	public void setUp() {
        String configLocation="spring/spring-redis2.xml";
        ApplicationContext context =new ClassPathXmlApplicationContext(configLocation);
        jedisPool = (JedisPool) context.getBean("jedisPool");
//		pool = new JedisPool(new JedisPoolConfig(), "172.16.200.100", 6379);
...
//      jedis.auth("password");
	...
	public void testBasicString() {
		...
		// -----修改数据-----------
		// 1、在原来基础上修改
		jedis.append("name", "jarorwar");       // 很直观，类似map 将jarorwarappend到已经有的value之后
		...
		jedis.mset("name", "minxr", "jarorwar", "闵晓荣"); // mset相当于 jedis.set("name","minxr"); jedis.set("jarorwar","闵晓荣");
	...
	public void testSet() {
		...
		System.out.println(jedis.srandmember("sname"));         // 是否是sname集合的元素
	...
	public void test() throws InterruptedException {
		...
		System.out.println(jedis.get("time"));      // 因为将timekey 重命名为time 所以可以取得值tmin
